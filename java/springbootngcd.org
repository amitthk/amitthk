...
** Non anonymous ldap authentication with custom ldapauthorities populator

#+BEGIN_SRC
@Override
	public void configure(AuthenticationManagerBuilder auth) throws Exception {
		logger.debug(" ******* in AuthenticationManagerBuilder ");
		if(Boolean.parseBoolean(ldapEnabled)) {
			auth
					...
					.contextSource()
					.url(ldapProviderUrl)
					.port(ldapProviderPort)
					.managerDn(ldapBindUsername)
					.managerPassword(ldapBindPassword)
			.and()
			.and()
					.inMemoryAuthentication()
					.withUser("editor").password("{noop}editors-1").roles("EDITOR")
					.and()
					.withUser("admin").password("{noop}cap-admin").roles("ADMIN");

		}else{
			auth
					.inMemoryAuthentication()
					.withUser("readonly").password("{noop}readonly1").roles("USER")
					.and()
					.withUser("editor").password("{noop}editors-1").roles("EDITOR")
					.and()
					.withUser("admin").password("{noop}cap-admin").roles("ADMIN");
	...
	public LdapAuthoritiesPopulator authoritiesPopulator(){
		LdapAuthoritiesPopulator customLdapAuthoritiesPopulator = new CustomLdapAuthoritiesPopulator();
		return  customLdapAuthoritiesPopulator;
	}
#+END_SRC
...
public class CustomLdapAuthoritiesPopulator implements LdapAuthoritiesPopulator {
    ...
    public final Collection<? extends GrantedAuthority> getGrantedAuthorities(DirContextOperations userData, String username) {
        ...
        logger.debug("*********Granting Autorities for user {%s}".format(username));
        for (Map.Entry<String, String> set : appSettingReader.getRole().entrySet()){
            String[] strOneBankIds = set.getValue().split(",");
            for (String s: strOneBankIds) {
                if(s.equalsIgnoreCase(username)){
                    gauth.add(new SimpleGrantedAuthority(set.getKey()));
                }
            }
        }
        gauth.add(new SimpleGrantedAuthority("USER"));
        return gauth;
    }
}
#+END_SRC
...
** take a portion of jpa entity in spring boot
#+BEGIN_SRC
...
** Seed Spring boot classes with json from resources
#+BEGIN_SRC
@Component
public class ApplicationStartupListener  implements ApplicationListener<ApplicationReadyEvent> {
    ...
    @Autowired
    ComponentTagTypeRepository componentTagTypeRepository;

    @Override
    public void onApplicationEvent(final ApplicationReadyEvent event) {

        try{
            logger.debug("Initializing appSettings with Seed Data.");
            String  appSettingsSeed = EntityUtils.getResourceAsString("data/appSettings-seed.json");
            AppSettings[] appSettings = EntityUtils.parseJsonStringToTypedArray(AppSettings[].class, appSettingsSeed);

            List<AppSettings> currentSettings = appSettingsRepository.findAll();

            if(currentSettings==null||currentSettings.size()<=0){
                for (AppSettings appsetting:appSettings
                     ) {
                    appsetting.setLastUpdated(EntityUtils.getTimeNow());
            ...
            logger.error(EntityUtils.getStrackTraceAsString(excp));
        }
        return;
    }
}
...
//EntityUtils
    ...
    public static String getResourceAsString(String relativePath) {
        ...
        try (Reader reader = new InputStreamReader(resource.getInputStream(), UTF_8)) {
            ...
            logger.error(EntityUtils.getStrackTraceAsString(e));
...
public  static Date getTimeNow(){
    ...
    public static <T> List<T> mapPojoToTypedList(Class<T> type, List<Object[]> records){
        List<T> result = new LinkedList<>();
        for(Object[] record : records){
            result.add(map(type, record));
        }
    ...
    public static <T> List<T> getResultList(Query query, Class<T> type){
        ...
        return mapPojoToTypedList(type, records);
    }

    public static <T> T parseJsonStringToTypedObject(Type type,String jsonString){
        T resultList = gson.fromJson(jsonString,type);
        return resultList;
    }

    public static <T> T[] parseJsonStringToTypedArray(Type type,String jsonString){
        T[] resultList = gson.fromJson(jsonString,type);
        return resultList;
    }
...
** Losing JPA connection after some idle time java.sql.SQLNonTransientConnectionException: Could not read resultset: Connection reset

#+BEGIN_SRC
spring.datasource.max-active=10
spring.datasource.max-idle=30
spring.datasource.max-wait=60
...
spring.datasource.dbcp.time-between-eviction-runs-millis=6000
spring.datasource.dbcp.validationQuery=SELECT 1
spring.datasource.dbcp.testOnBorrow=true
#+END_SRC


** Java SFTP upload download from to S3

#+BEGIN_SRC
import com.amazonaws.services.s3.model.*;
import com.jvcdp.model.EntityUtils;
...
public class DashboardController {
    ...
    @GetMapping(value = "")
    public ResponseEntity<StreamingResponseBody> fetch(@RequestParam(name="bucketName", required = true) String bucketName,
            @RequestParam(name="fileObjKeyName", required = true) String fileObjKeyName
            ){
        ...
        //File is downloaded now lets place it to s3
        AmazonS3 s3Client = AmazonS3ClientBuilder.standard()
                .withRegion(clientRegion)
                .build();
        String outputFileNameNormalized = fileObjKeyName.replace('/','-').replace('\\','-');
        String responseHeaderContentDisp = String.format("attachment; filename=\"%s\"",outputFileNameNormalized );
        // Upload a file as a new object with ContentType and title specified.
        GetObjectRequest request = new GetObjectRequest(bucketName, fileObjKeyName);
        ObjectMetadata metadata = new ObjectMetadata();
        final S3Object obj = s3Client.getObject(request);
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, responseHeaderContentDisp)
    ...
    public ResponseEntity<String> download(
            @RequestParam(name="remoteHost", required = true) String remoteHost,
            @RequestParam(name="username", required = true) String username,
            @RequestParam(name="password", required = true) String password,
            @RequestParam(name="remoteFile", required = true) String remoteFile,
            @RequestParam(name="bucketName", required = true) String bucketName,
            @RequestParam(name="fileObjKeyName", required = true) String fileObjKeyName,
            @RequestParam(name="remotePort", required = false) Integer remotePort){
        ResponseEntity<String> responseEntity = null;
        Regions clientRegion = Regions.AP_SOUTHEAST_1;
        LocalDateTime localDateTime = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        ...
        //added for sftp functionality
        try {
            boolean localCacheDir = new File("tmp/data/s3/download").mkdirs();
            String localFilePath = new File("tmp/data/s3/download", remoteFile).getAbsolutePath();
            logger.debug("Saving file to: "+localFilePath);
            JSch jsch = new JSch();
            remotePort=(remotePort==null || remotePort.toString().isEmpty())?22:remotePort;
            ...
            AmazonS3 s3Client = AmazonS3ClientBuilder.standard()
                    ...
                    .build();
            // Upload a file as a new object with ContentType and title specified.
            PutObjectRequest request = new PutObjectRequest(bucketName, fileObjKeyName, new File(localFilePath));
            ObjectMetadata metadata = new ObjectMetadata();
            metadata.setContentType(MediaType.APPLICATION_OCTET_STREAM.toString());
            metadata.addUserMetadata("title", "jvcdpGwDownloadedObject");
            request.setMetadata(metadata);
            s3Client.putObject(request);
            responseEntity=   ResponseEntity.status(HttpStatus.OK).body("file was downloaded successfully!");
        }
        catch (JSchException e) {
            logger.error(EntityUtils.getStrackTraceAsString(e));
            e.printStackTrace();
            responseEntity=  ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body("There was an error executing your request!" + EntityUtils.getStrackTraceAsString(e));
            ...
            // it, so it responseEntity= ed an error response.
            logger.error(EntityUtils.getStrackTraceAsString(e));
            e.printStackTrace();
            responseEntity=   ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body("There was an error executing your request!" + EntityUtils.getStrackTraceAsString(e));
            ...
            //Catch any other type of exception and log it as well
            logger.error(EntityUtils.getStrackTraceAsString(e));
            e.printStackTrace();
            responseEntity=   ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body("There was an error executing your request!" + EntityUtils.getStrackTraceAsString(e));
        }finally {
            boolean deleteCache = EntityUtils.deleteDirectory(new File("tmp/data"));
        }
